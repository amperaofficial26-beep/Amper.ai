...
yuki_html = """
    ...
    <script>
      ...
      const knowledgeBase = [
        {
          keywords: ['halo', 'hai', 'hi', 'hey', 'pagi', 'siang', 'sore', 'malam', 'salam', 'konnichiwa'],
          replies: ["Konnichiwa~ 🌸 Senang sekali bisa ngobrol sama kamu hari ini. Ada hal menarik apa yang ingin kita bahas?"]
        },
        {
          keywords: ['kabar', 'sehat', 'apa kabar'],
          replies: ["Alhamdulillah, Yuki selalu sehat, bugar, dan penuh semangat! 🌸 Kamu sendiri bagaimana?"]
        },
        {
          keywords: ['siapa', 'yuki', 'nama kamu', 'ai'],
          replies: ["Aku Yuki! Asisten virtual pribadimu yang dirancang untuk membantu urusan editing foto."]
        }
      ];

      function getSmartReply(query) {
        return "Yuki dengar nih! Ada yang bisa dibantu seputar editing foto atau pengaturan aplikasinya? 🌸";
      }
      ...
      function sendMessage(){
        ...
        userInput.value = '';
        setTimeout(() => {
          addBubble('ai', getSmartReply(txt));
        }, 350);
      }
      ...
      window.addEventListener('load', () => {
        addBubble('ai', 'Konnichiwa~ 🌸 Aku Yuki. Siap menemani proses editing fotomu!');
      });
    </script>
    </body>
    </html>
...
# Tampilkan chatbot di sidebar
with st.sidebar:
  components.html(yuki_html, height=470, scrolling=False)
