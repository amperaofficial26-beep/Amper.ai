...
with st.sidebar:
    ...
    exposure = st.slider("Exposure", -2.0, 2.0, 0.0, key="exposure", step=0.05)
    contrast = st.slider("Contrast", -50, 50, 0, key="contrast", step=1)
    highlights = st.slider("Highlights", -100, 100, 0, key="highlights", step=1)
    shadows = st.slider("Shadows", -100, 100, 0, key="shadows", step=1)
    whites = st.slider("Whites", -50, 50, 0, key="whites", step=1)
    blacks = st.slider("Blacks", -50, 50, 0, key="blacks", step=1)

    st.markdown("### 2. Color & White Balance")
    temp = st.slider("Temperature (Kelvin/Tint)", -50, 50, 0, key="temp", step=1)
    tint = st.slider("Tint", -50, 50, 0, key="tint", step=1)
    vibrance = st.slider("Vibrance", -50, 50, 0, key="vibrance", step=1)
    saturation = st.slider("Saturation", -50, 50, 0, key="saturation", step=1)

    st.markdown("### 3. Detail, Clarity & Effects")
    clarity = st.slider("Clarity / Texture", -50, 50, 0, key="clarity", step=1)
    dehaze = st.slider("Dehaze", -50, 50, 0, key="dehaze", step=1)
